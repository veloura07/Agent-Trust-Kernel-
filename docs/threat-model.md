# AgentGuard v12 Threat Model & Security Boundaries

## 1. Architectural System Boundaries
AgentGuard provides a local-first, edge-verified runtime security envelope for autonomous entities. It enforces authorization boundaries at the intercept layer before tool mutation execution occurs.

```text
[Untrusted Agent Process Space] ──(SDK Guard Boundary)──> [AgentGuard Microkernel Core]
                                                                        │
...
* **Does NOT Defend Against (Out of Scope):**
  ...
  * **Malicious LLM Outputs:** AgentGuard restricts system *actions and resource mutations*, not the underlying cognitive accuracy of semantic generations.
